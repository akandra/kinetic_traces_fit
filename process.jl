function process(df2fit::DataFrame, kinetic_traces::Vector{Matrix{Float64}})
    ...
    for i in 1:ndata
        ...
        display(plot( kinetic_traces[i][:,1], [kinetic_traces[i][:,2], yfit1, [df2fit1.baseline[1].value],[xdata1[df2fit1[1,:cutoff]]]],
            ...
            ann=append!(
                [((0.7,0.85-0.05*j),
                    ann_par(k,  df2fit[:,k][i])) for (j,k) in enumerate(rate_constants)],
                [((0.7,0.84-0.05*size(rate_constants,1)-0.05*j),
                    ann_par(f,  df2fit[:,f][i])) for (j,f) in enumerate(fit_parnames[1:3])],
